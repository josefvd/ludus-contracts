...
# Find the certora executable within the venv
CERTORA_EXEC="$VENV_DIR/bin/certora"
if [ ! -x "$CERTORA_EXEC" ]; then
    echo "ERROR: Could not find certora executable in $VENV_DIR/bin"
    deactivate
    exit 1
fi

# Run verification focusing on the treasury distribution and event timeline rules
echo "Running validation of treasury distribution and event timeline rules using $CERTORA_EXEC..."
"$CERTORA_EXEC" run packages/foundry/certora/certora_events.conf
