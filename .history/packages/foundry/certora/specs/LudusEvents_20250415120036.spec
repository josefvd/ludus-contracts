...
rule onlyOwnerCanCancelEvent(uint256 eventId) {
    ...
    // Define helper function inside the rule
    ghost bool isValidTreasuryDistribution(uint256 _eventId) {
        // Temporarily commented out body to isolate syntax issues
        // var (_, _, _, _, _, _, _, _, _, athletesShare, organizerShare, charityShare, _, _) = getEventDetails(_eventId);
