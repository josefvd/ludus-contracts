...
# Use absolute path for config file (corrected)
CONFIG_FILE_PATH="$(pwd)/packages/foundry/certora/certora_events.conf"
# The script cds to the root, so the path should be relative from there
CONFIG_FILE_PATH="packages/foundry/certora/certora_events.conf"
