...
contract YieldManager is Ownable, IYieldManager {
    ...
    function stakeEventFundsUSDC(uint256 eventId, uint256 amount) external override {
        require(msg.sender == events, "Only events contract can stake funds");
    ...
    function withdrawEventFundsUSDC(uint256 eventId) external returns (uint256 totalAmount) {
        require(msg.sender == events, "Only events contract can withdraw funds");
        ...
        require(_USDC.transfer(events, amountToReturn), "USDC transfer failed");
    ...
    function withdrawEventFundsETH(uint256 eventId) external returns (uint256 totalAmount) {
        ...
        (bool success, ) = events.call{value: amountToReturn}("");
    ...
    function emergencyWithdraw(uint256 eventId) external onlyOwner {
        ...
        if (hasUSDC) {
            ...
            require(_USDC.transfer(events, eventYield.stakedAmount), "USDC transfer failed");
        ...
        if (hasETH) {
            ...
            (bool success, ) = events.call{value: eventYield.stakedAmountETH}("");
            require(success, "ETH transfer failed");
    ...
    function setDistribution(
        ...
        if (!isTestMode) {
            require(msg.sender == events, "Only events contract can set distribution");
        }
