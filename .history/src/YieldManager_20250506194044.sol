    ...
    function enableYieldGeneration(uint256 eventId) external {
        ...
        if (eventYield.stakedAmount > 0 && !eventYield.isGeneratingYield) {
            ...
            if (currentAllowance < eventYield.stakedAmount) {
                IERC20 localUSDC = _USDC; // Assign to local variable
                localUSDC.safeApprove(AAVE_POOL, 0); // Reset approval
                localUSDC.safeApprove(AAVE_POOL, type(uint256).max); // Approve maximum allowance
            }
        ...
        if (eventYield.stakedAmountETH > 0 && !eventYield.isGeneratingYieldETH) {
            ...
            if (currentAllowance < eventYield.stakedAmountETH) {
                SafeERC20.safeApprove(_WETH, AAVE_POOL, 0); // Reset approval
                SafeERC20.safeApprove(_WETH, AAVE_POOL, type(uint256).max); // Approve maximum allowance
            }
    ...
    function stakeEventFunds(uint256 eventId, uint256 amount) external override {
        ...
        if (!isTestMode) {
            ...
            if (currentAllowance < amount) {
                SafeERC20.safeApprove(_USDC, AAVE_POOL, 0);
                SafeERC20.safeApprove(_USDC, AAVE_POOL, type(uint256).max);
            }
    ...
    function stakeEventFundsETH(uint256 eventId) external payable {
        ...
        if (!isTestMode) {
            ...
            if (currentAllowance < msg.value) {
                SafeERC20.safeApprove(_WETH, AAVE_POOL, 0);
                SafeERC20.safeApprove(_WETH, AAVE_POOL, type(uint256).max);
            }
