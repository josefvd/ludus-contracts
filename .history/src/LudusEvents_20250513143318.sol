...
abstract contract LudusEvents is Ownable, ReentrancyGuard, ISchemaResolver {
    ...
    function _calculateTotalPrizePool(
        ...
        totalPrizePool = params.registrationFeeUSDC * params.maxParticipants;
    ...
    function registerParticipant(uint256 eventId, uint256 profileId) external payable {
        ...
        if (msg.value > 0) {
            ...
            if (eventData.preferYieldGeneration) {
                ...
                eventPrizesETH[eventId] += msg.value;
            }
        } else {
            ...
            eventPrizesUSDC[eventId] += eventData.registrationFeeUSDC;
            
            if (eventData.preferYieldGeneration) {
                // Assumes YieldManager has stakeEventFundsUSDC and appropriate approvals are set
                yieldManager.stakeEventFundsUSDC(eventId, eventData.registrationFeeUSDC);
                emit USDCStaked(eventId, eventData.registrationFeeUSDC);
            } else {
                eventPrizesUSDC[eventId] += eventData.registrationFeeUSDC;
            }
        }
