...
abstract contract LudusEvents is Ownable, ReentrancyGuard, ISchemaResolver {
    ...
    // Internal function to validate inputs for createEvent
    ...
    // Internal function to calculate the total prize pool based on fees and attestation data
    function _calculateTotalPrizePool(
        ...
        totalPrizePool = registrationFeeUSDC * maxParticipants;
        ...
        // Return value is implicitly totalPrizePool
    }

    // Internal function to store core event details and distributions
    ...
    // Internal function to handle YieldManager updates and EAS Attestation
    function _handleYieldAndAttestation(
        ...
        uint256 charityShare,
        address charityAddress,
        LudusTypes.EventAttestationData memory attestationData // Pass struct by memory
    ) internal {
        if (block.chainid != 31337) {
            ...
            // Note: Modifying the memory struct here is fine as it's a local copy 
            // within this function call stack, but be mindful if passing storage refs.
            ...
            bytes32 attestationUid = LudusAttestations.createEventAttestation(
                ...
                msg.sender, // Attester is msg.sender (organizer)
                eventSchemaId, // Access state variable
                address(eas)   // Access state variable
            );
            ...
            try eas.getAttestation(attestationUid) returns (Attestation memory attestation) {
                ...
                // Updated check: Attester should be msg.sender (the one who called createEvent)
    ...
    function createEvent(
        ...
        _validateCreateEventInputs(
            startTime, endTime, registrationStartTime, registrationEndTime, maxParticipants,
            registrationFeeUSDC, registrationFeeETH, positions, percentages, athletesShare,
            organizerShare, charityShare, refereeAddress, organizerAddress, attestationData
        );
        
        eventCount++;
        uint256 currentEventId = eventCount; 

        // Calculate total prize pool using the internal function
        uint256 totalPrizePool = _calculateTotalPrizePool(
            registrationFeeUSDC, 
            registrationFeeETH, 
            maxParticipants, 
            attestationData
        ...
        // Store event details using the internal function
        _storeEventDetails(
            currentEventId, startTime, endTime, registrationStartTime, registrationEndTime,
            maxParticipants, organizerAddress, refereeAddress, registrationFeeETH,
            registrationFeeUSDC, totalPrizePool, preferYieldGeneration, athletesShare,
            organizerShare, charityShare, charityAddress, positions, percentages
        );
        ...
        _handleYieldAndAttestation(
            ...
            charityShare, 
            charityAddress, 
            attestationData // Pass the memory struct
        );
        
        // Emit final events
        emit EventCreated(currentEventId, attestationData.name, startTime, endTime);
        emit DistributionSet(currentEventId, athletesShare, organizerShare, charityShare, charityAddress);
