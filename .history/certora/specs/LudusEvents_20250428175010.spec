...
using LudusEvents { // Specify the contract context
    // Declare the mapping; the struct type is inferred from the contract
...
invariant validEventTimelineInvariant()
    forall uint256 eventId in events.keys() {
        ...
        // Only check events that presumably exist (startTime is set)
        // If startTime can be 0 for valid non-started events, adjust logic
