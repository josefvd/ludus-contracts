...
methods {
    ...
    function schemaRegistry() external returns (address) envfree; // Added schemaRegistry
    function USDC() external returns (address) envfree; // Added USDC accessor if needed for harness linking


    // --- Declarations of External Function Signatures Called by LudusEventsImpl ---
    // These might be needed if rules interact with them, or for summary; otherwise, linking is enough.
