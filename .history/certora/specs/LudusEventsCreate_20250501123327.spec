...
// Simplified rule signature - implicitly applies only when createEvent is called and returns eventId
rule createEventPostConditionCheck(
    bytes configBytes,
    bytes treasuryDistBytes,
    bytes athleteDistBytes,
    bytes attestationDataBytes
) returns uint256 eventId // Capture the return value directly
{
    env e; // Access environment variables like msg.sender

    // Rule implicitly requires createEvent to have run successfully to get here
    // and returned a non-zero eventId. Add explicit check anyway.
