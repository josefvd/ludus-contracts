...
methods contract LudusIdentityContract {
...
methods {
    ...
    function createEvent(bytes memory, bytes memory, bytes memory, bytes memory) returns (uint256) summary;
    function startEvent(uint256) summary;
    function completeEvent(uint256, address[] memory, uint256[] memory) summary;
    ...
    function ludusIdentity() external returns (LudusIdentityContract) envfree;

    // Calls to External Contracts (Summarized)
    // Note: These might need more specific summaries depending on invariant needs
    function addToBalanceOf(uint256, address, uint256, string memory) summary;
    function setDistributionCurrentEventId(uint256) summary;

    // Removed declarations for ownerOf, canEnterTournament, etc. as they are either
    // part of LudusIdentityContract definition or no longer exist on LudusIdentity
}
