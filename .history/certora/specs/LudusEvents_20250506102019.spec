...
methods {
    ...
    // --- Declarations for external contract addresses (assuming public state vars) ---
    ...
    // Summary for jbTerminal.addToBalanceOf(...)
    // Assuming it has no side effects relevant to LudusEventsImpl invariants and is void or its return is unused.
    function jbTerminal.addToBalanceOf(uint256, address, uint256, string) external {}

    // Summary for yieldManager.setDistributionCurrentEventId(...)
    // Assuming it has no side effects relevant to LudusEventsImpl invariants and is void or its return is unused.
