...
function eventExistsAndTimelineValid(uint256 eventId) returns bool {
    return (let (
        id_ev, startTime_ev, endTime_ev, regStartTime_ev, regEndTime_ev,
        maxParticipants_ev, minParticipantsToStart_ev, creator_ev, referee_ev, status_ev,
        regFee_ev, paymentCurrency_ev, totalPrize_ev, prefYield_ev, yieldActivated_ev
    ) = events(eventId) in (id_ev == eventId && id_ev > 0) ? checkTimelineProperty(eventId) : true);
}
...
invariant validTimeline(uint256 eventId)
    let (
        ...
        (id_ev == eventId && id_ev > 0) ? checkTimelineProperty(eventId) : true;