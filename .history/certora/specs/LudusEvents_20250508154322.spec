...
methods {
    ...
    function createEvent(bytes memory, bytes memory, bytes memory, bytes memory) external returns (uint256) summary;
    function startEvent(uint256) external summary;
    function completeEvent(uint256, address[] memory, uint256[] memory) external summary;
    ...
    function addToBalanceOf(uint256, address, uint256, string memory) external summary;
