...
methods {
    ...
    function createEvent(bytes, bytes, bytes, bytes) returns (uint256);
    function startEvent(uint256);
    function completeEvent(uint256, address[], uint256[]);
    function cancelEvent(uint256);
    function enableETHYieldGeneration(uint256);
    function registerParticipant(uint256,uint256);
    function withdrawEventETH(uint256);
