...
abstract contract Stub_SchemaRegistry is ISchemaRegistry {
    ...
    function getSchema(bytes32 schemaUID) external view returns (SchemaRecord memory) {
        // Not directly called by LudusEvents logic for these invariants.
        return SchemaRecord({uid: schemaUID, resolver: ISchemaResolver(address(0)), revocable: false, schema: ""});
    }

    function getSchema(string calldata) external view returns (SchemaRecord memory) {
        // Not directly called by LudusEvents logic for these invariants.
