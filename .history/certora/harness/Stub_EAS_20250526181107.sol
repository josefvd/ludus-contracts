...
abstract contract Stub_EAS is IEAS {
    ...
    function timestamp(bytes32) external returns (uint64) {
        return uint64(block.timestamp);
    }

    function multiTimestamp(bytes32[] calldata) external returns (uint64) {
        return uint64(block.timestamp);
    }

    function revokeOffchain(bytes32) external returns (uint64) {
        return uint64(block.timestamp);
    }

    function multiRevokeOffchain(bytes32[] calldata) external returns (uint64) {
        return uint64(block.timestamp);
    }
    ...
    function isAttestationValid(bytes32) external view returns (bool) {
        return true; // Simplification for harness
    }

    function getTimestamp(bytes32) external view returns (uint64) {
        return uint64(block.timestamp);
    }

    function getRevokeOffchain(address, bytes32) external view returns (uint64) {
        return uint64(block.timestamp);
    }
