    ...
    function attest(Attestation calldata attestation) external payable override returns (bool) {
        ...
        if (attestation.schema == eventSchemaId) {
            ...
            for (uint256 i = 0; i < resultData.winnerIds.length; i++) {
                finalizedAthletes[i] = ludusIdentity.ownerOf(resultData.winnerIds[i]);address certora_local41 = finalizedAthletes[i];assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff00000029,certora_local41)}
