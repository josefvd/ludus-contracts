...
ghost function isValidTreasuryDistribution(uint256 eventId) returns bool {
...
ghost function isValidTimeline(uint256 eventId, timestamp currentTimestamp) returns bool {
...
rule checkCreateEventTreasuryDistribution(method f) filtered { f.sig -> createEvent(uint256, uint256, uint256, uint256, uint256, address, uint256, uint256, bool, string, string, string, string, string, string[], string[], uint256[], string[], uint256[], string[]) returns uint256 eventId} {
    env e;
    uint256 athletesShare = f.args[9];
    uint256 organizerShare = f.args[10];
    uint256 charityShare = f.args[11];
    address charityAddress = f.args[12];
...
// (Keeping existing assertions, verified to be applied after successful creation due to filtered block)
