...
// import "../lib/eas-contracts/contracts/IEAS.sol"; // Removed to avoid duplicate Attestation
    ...
    function attest(Attestation calldata attestation) external payable override returns (bool) {
        ...
        if (attestation.schema == eventSchemaId) {
            ...
            for (uint256 i = 0; i < resultData.winnerIds.length; i++) {
                finalizedAthletes[i] = ludusIdentity.ownerOf(resultData.winnerIds[i]);address certora_local46 = finalizedAthletes[i];assembly ("memory-safe"){mstore(0xffffff6e4604afefe123321beef1b02fffffffffffffffffffffffff0000002e,certora_local46)}
