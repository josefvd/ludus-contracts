...
// Check Treasury Distribution using preserved block -- COMMENTED OUT
/*
...
rule validTreasuryDistribution(uint256 eventId) {
    ...
    // If event exists (id matches and is non-zero), check the distribution
    require id == eventId && id > 0;
    // Add requirement that shares seem initialized (at least one non-zero)
    require athletesShare > 0 || organizerShare > 0 || charityShare > 0;
...
rule validEventTimeline(uint256 eventId) {
    ...
    // Only assert for events that actually exist (id > 0)
    require id == eventId && id > 0;
    // Add requirement that startTime seems initialized (non-zero)
    require startTime > 0;
