...
// using LudusTypes as types; // Commented out again
...
// Helper function to check Treasury Distribution for a given eventId
function checkTreasuryDistribution(uint256 eventId) returns bool {
    ...
    // Property holds if event doesn't exist OR if it exists and distribution is valid
    return (id != eventId || id == 0) || (
        (athletesShare + organizerShare + charityShare == 9700) &&
        (charityShare == 0 || charityAddress != 0)
    );
}

// Helper function to check Timeline for a given eventId
function checkTimeline(uint256 eventId) returns bool {
    ...
    uint256 regEndTime;
    // ... other fields ...
    LudusTypes.EventStatus status; // Status might be useful for context
    // Assign results based on order
    (id, startTime, endTime, regStartTime, regEndTime, _, _, _, status, _, _, _, _) = events(eventId);

    // Property holds if event doesn't exist OR if it exists and timeline is valid
    return (id != eventId || id == 0) || (
        regEndTime <= startTime && endTime > startTime
    );
}

// Invariant 1: Valid Treasury Distribution (using helper function)
invariant validTreasuryDistributionInvariant(uint256 eventId) {
    assert checkTreasuryDistribution(eventId);
}

// Invariant 2: Valid Event Timeline (using helper function)
invariant validTimelineInvariant(uint256 eventId) {
    assert checkTimeline(eventId);
}
